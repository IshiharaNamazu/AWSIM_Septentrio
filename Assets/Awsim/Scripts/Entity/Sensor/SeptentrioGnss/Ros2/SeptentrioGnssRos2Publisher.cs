...
// using septentrio_gnss_driver.msg;
...
{
    public class SeptentrioGnssRos2Publisher : MonoBehaviour
    {
        [Header("NavSatFix Output")]
        [SerializeField] string _navSatFixTopic = "/sensing/gnss/septentrio/nav_sat_fix";
        // [SerializeField] string _attEulerTopic = "/sensing/gnss/septentrio/atteuler";
        [SerializeField] string _orientationTopic = "/sensing/gnss/septentrio/orientation";
        ...
        struct SeptentrioMsgs
        {
            ...
            //public AttEuler AttEuler;
            public GnssInsOrientationStamped Orientation;
        };
        SeptentrioMsgs _tmpData;
        ...
        void Reset()
        {
            var instance = GetComponent<SeptentrioGnssSensor>();
            if (instance != null)
            {
        ...
        void DataUpdate(SeptentrioGnssSensor.IReadOnlyOutputData data)
        {
            ...
            /*
            _tmpData.AttEuler.Heading = data.AttEuler.heading;
            _tmpData.AttEuler.Pitch = data.AttEuler.pitch;
            _tmpData.AttEuler.Roll = data.AttEuler.roll;
            _tmpData.AttEuler.Heading_dot = data.AttEuler.heading_dot;
            _tmpData.AttEuler.Pitch_dot = data.AttEuler.pitch_dot;
            _tmpData.AttEuler.Roll_dot = data.AttEuler.roll_dot;
            */
            ...
            // AwsimRos2Node.UpdateROSTimestamps(_tmpData.NavSatFix as MessageWithHeader, _tmpData.AttEuler as MessageWithHeader);
            AwsimRos2Node.UpdateROSTimestamps(_tmpData.NavSatFix as MessageWithHeader, _tmpData.Orientation as MessageWithHeader);
        ...
        void HighFrequencyUpdate()
        {
            ...
            var pqMsg = new SortedDictionary<(int, uint, int), SeptentrioMsgs>(); // priority queue(SortedDictionaryで代用)
            ...
            {
                Thread.Sleep(1000 / _highFreqUpdateHz);
                _pqmsg_size = pqMsg.Count;

                if (_dataReady)
                {
                    // _tmpDataのディープコピーを作成してキューに追加する
                    var newMsg = new SeptentrioMsgs
                    {
                        ...
                        // AttEuler = CreateDeepCopyAttEuler(_tmpData.AttEuler)
        ...
        private AttEuler CreateDeepCopyAttEuler(AttEuler original)
        {
            var newAttEuler = new AttEuler
            {
                ...
                Roll = original.Roll,
                Heading_dot = original.Heading_dot,
                Pitch_dot = original.Pitch_dot,
                Roll_dot = original.Roll_dot
            };
